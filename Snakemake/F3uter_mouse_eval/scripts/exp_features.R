...
for(tissue in tissues){
  ...
  print(str_c(Sys.time(), " - calculating expression features"))
  ...
  for(i in c(1:19,"X","Y")){
    ...
    # per chr
    chr.er = er %>% dplyr::filter(seqnames %in% i)

    if(nrow(chr.er) > 0) {

      # load exp data
      file = paste(coverage_dir, "/", tissue, "_chr", i, "_mean_cov.rda", sep="")
  ...
  er.export = er.exp %>%
    dplyr::select(c(id, meanCov, pd, EE, class))
