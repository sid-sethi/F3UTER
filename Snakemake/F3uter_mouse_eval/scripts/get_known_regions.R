...
gtf = keepStandardChromosomes(gtf, species = "Mus_musculus", pruning.mode="coarse")
...
# Select lncRNA
lncrna.gtf = gtf.exp %>% dplyr::filter(gene_biotype %in% lncRNA, type %in% "exon")
lncrna.gtf$transcript_support_level = gsub("\\s*\\([^\\)]+\\)","",as.numeric(lncrna.gtf$transcript_support_level))
lncrna.gtf <- lncrna.gtf %>% dplyr::filter(transcript_support_level %in% 1)
...
lncrna_all_collapse = IRanges::reduce(lncrna_all_grList, with.revmap=TRUE) %>% as.data.frame() %>%
  mutate(elements_collapsed = lengths(revmap), id = paste(group_name, seqnames, start, end, strand, elements_collapsed, sep=":"), class = "lncrna")
...
pseudo.gtf$transcript_support_level = gsub("\\s*\\([^\\)]+\\)","",as.numeric(pseudo.gtf$transcript_support_level))
pseudo.gtf <- pseudo.gtf %>% dplyr::filter(transcript_support_level %in% 1)
...
data <- rbind(utr, ice, five, lncrna, ncrna, pseudogene) %>% dplyr::distinct(id, .keep_all=TRUE)
