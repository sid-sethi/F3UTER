...
utr_all = gtf.df %>% dplyr::filter(type %in% "three_prime_utr")
...
er.hits <- er.gr[unique(queryHits(hits))] %>% as.data.frame()
  ...
  biotype <- table(utr.hits$transcript_biotype) %>% prop.table() %>% as.data.frame() %>%
    mutate(
  ...
  write.table(er.hits, str_c(outpath, "/", prefix, "_ers_ensembl.txt"), quote=FALSE, row.names=FALSE, col.names=TRUE, sep="\t")
  write.table(df, str_c(outpath, "/", prefix, "_overlap_ensembl.txt"), quote=FALSE, row.names=FALSE, col.names=TRUE, sep="\t")
  write.table(biotype, str_c(outpath, "/", prefix, "_biotype_ensembl.txt"), quote=FALSE, row.names=FALSE, col.names=TRUE, sep="\t")
  write.table(tsl, str_c(outpath, "/", prefix, "_tsl_ensembl.txt"), quote=FALSE, row.names=FALSE, col.names=TRUE, sep="\t")



# matching gene names
er.df <- er.gr[queryHits(hits)] %>% as.data.frame()
utr.df <- utr.gr[subjectHits(hits)] %>% as.data.frame() %>% rename_with(~str_c("gtf_", .x))
...
write.table(same_gene.df, str_c(outpath, "/", prefix, "_geneConcordance_ensembl.txt"), quote=FALSE, row.names=FALSE, col.names=TRUE, sep="\t")
