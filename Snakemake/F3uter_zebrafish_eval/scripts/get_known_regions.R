...
chrs <- c(1:25, "MT")
# gtf = keepStandardChromosomes(gtf, species = "Danio_rerio", pruning.mode="coarse") # not available for zebrafish
gtf <- gtf %>% plyranges::filter(seqnames %in% chrs)
...
lncrna_all_collapse = IRanges::reduce(lncrna_all_grList, with.revmap=TRUE) %>% as.data.frame() %>%
  mutate(elements_collapsed = lengths(revmap), id = paste(group_name, seqnames, start, end, strand, elements_collapsed, sep=":"), class
 = "lncrna")
