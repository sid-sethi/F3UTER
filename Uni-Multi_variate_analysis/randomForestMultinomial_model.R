...
for(j in 1:5){
  ...
  model<-train(class~.,data=train,method="rf",metric = "Kappa", trControl=cntrl,importance = TRUE)



  predicted.response <- predict(model, test)
  confuseMat.test = confusionMatrix(data=as.factor(predicted.response),reference=test$class)
  #confuseMat.test
